...
/**
 * The AVL<E> class is an implementation of a self-balancing binary search tree. This 
 * implementation provides guaranteed O(H) (H is the height of this tree which is as 
 * low as logN and is the time cost for the basic operations (add, remove and contains). 
 * This class implements many of the methods provided by the Java framework's TreeSet 
 * class. Note that in the documentations of this class's methods, M is the size of the
 * collection. Some parts and concepts of this code were understood and adapted from popular websites
 * like GeeksForGeeks, StackOverflow and other websites. 
 * @author  Mofoluwake Adesanya
 * @version 11/29/2023
 */
public class AVL<E extends Comparable<E>> extends Object implements Iterable<E>, Cloneable {
	private Node<E> root;
	private int height = -1;
	private int size = 0;
	/**
	 * Constructs a new, empty balanced tree. All elements inserted into the tree 
	...
	/**
	 * Updates the height of the given node. This is calculated from the bottom of the 
	 * tree upwards.
	 * @param Node<E> node to return the height of 
	 * @return the height of the given node.
	 */
	private void updateHeight ( Node<E> node ) {
		if ( node != null ) {
			// height is not 0 if node is a leaf node
			if ( node.left == null && node.right == null )
				node.height = 0;
			// if node has no left child, use right child's height
			else if ( node.left == null ) 
				node.height = getHeight(node.right) + 1;
			// if node has no right child, use left child's height
			else if ( node.right == null )
				node.height = getHeight(node.left) + 1;
			// if it has both children, get the greater height
			else 
	...
	/**
	 * Returns the height of the given node. Updates the height of that node as well.
	 * @param Node<E> node to get height of
	 * @return the height of the given node.
	 */
	private int getHeight(Node<E> node) {
		int height = -1; //
		// only use the node that is not null
		if ( node.left != null )
			height = Math.max(height, getHeight(node.left));
		if ( node.right != null )
			height = Math.max(height, getHeight(node.right));
		// update the height here
		node.height = height + 1;
		return node.height;
	}
	/**
	 * Returns the balance factor of a given node. The balance factor of a node 
	 * is the difference of the heights of the left sub-tree and the right sub-tree of 
	 * a given node. This determines whether rotation(s) would occur. If it returns -2 or 2, 
	 * then a rotation is performed.
	 * @param Node<E> node to find the balance factor of
	 * @return the balance factor of the node
	 */
	private int balanceFactor ( Node<E> node ) {
		if ( node != null ) {
			// we chose right as negative weight
			if ( node.right == null )
				return node.height;
			// left is the positive weight
			if ( node.left == null )
				return -node.height;
			// the difference should balance each other 
			return node.left.height - node.right.height;
		} // if node is null
		return 0;
	}
	/**
	 * Balances this tree when the left subtree has 2 more levels than its right 
	 * subtree. A left subtree of the left subtree of node has a height one larger than 
	 * the right subtree. This is a single rotation.
	 * @param Node<E> node the root node of the subtree to balance
	 * @return the new root node of the subtree after rotating
	 */
	private Node<E> balanceLL (Node<E> node) {
		// save left child of node
		Node<E> left = node.left;
		// change the node's left child to the right child of its former left child
		node.left = left.right;
		// then the node becomes the right child of its former left child
		left.right = node;
		// update the height of each manipulated nodes 
		updateHeight(node);
		updateHeight(left);
		// returns a reference to the new root of the subtree after rotating
		return left;
	}
	/**
	 * Balances this tree when the left subtree has 2 more levels than its right 
	 * subtree. A right subtree of the left subtree of node has a height one larger
	 * than the right subtree. This is a double rotation : rotating left on the left subtree
	 * of the primary node and then right on the primary node.
	 * @param Node<E> node the root node of the subtree to balance
	 * @return the new root node of the subtree after rotating
	 */
	private Node<E> balanceLR (Node<E> node) {
		// save left child of node
		Node<E> left = node.left;
		// save the right subtree of left child of node
		Node<E> store = left.right; // store is where the imbalance occurs
		// set node's left child to the right child of store
		node.left = store.right;
		left.right = store.left; // change the right child of the left child of node to the left of store
		// store would even become the root of the subtree
		store.left = left;
		store.right = node;
		// update the height of each manipulated nodes 
		updateHeight(node);
	...
	/**
	 * Balances this tree when the right subtree has 2 more levels than its left 
	 * subtree. A right subtree of the right subtree of node has a height one larger
	 * than the left subtree.
	 * @param Node<E> node the root node of the subtree to balance
	 * @return the new root node of the subtree after rotating
	 */
	private Node<E> balanceRR (Node<E> node) {
		// save right child of node
		Node<E> right = node.right;
		// change the node's right child to the left child of its former right child
		node.right = right.left;
		// then the node becomes the left child of its former right child
		right.left = node;
		// update the height of each manipulated nodes 
		updateHeight(node);
		updateHeight(right);
	 ...
	 * Balances this tree when the right subtree has 2 more levels than its left 
	 * subtree. A right subtree of the left subtree of node has a height one larger
	 * than the right subtree. This is a double rotation : rotating right on the right subtree
	 * of the primary node and then left on the primary node.
	 * @param Node<E> node the root node of the subtree to balance
	 * @return the new root node of the subtree after rotating
	 */
	private Node<E> balanceRL (Node<E> node) {
		// save right child of node
		Node<E> right = node.right;
		// save the left subtree of right child of node
		Node<E> store = right.left; // store is where the imbalance occurs
		// set node's right child to the left child of store
		node.right = store.left;
		right.left = store.right; // change the left child of the right child of node to the right of store
		// store would even become the root of the subtree
		store.left = right;
		store.right = node;
		// update the height of each manipulated nodes 
	...
	public boolean add​(E e) throws NullPointerException {
		// add the node to the tree using super class method
		...
		// set rotations for specific inbalances
		// for left conditions: 
		if ( root.left != null && balance > 1 ) {
			// rotate left left 
			if (e.compareTo(root.left.data) == -1) 
			{	root = balanceLL(root);	}
			// rotate left right
			else if (e.compareTo(root.left.data) == 1) 
			{	root = balanceLR(root);	}
			// update this tree's height
			this.height = root.height;
			return true;
		} 
		// for right conditions:
		if ( root.right != null && balance < -1  ) {
			// rotate right right 
			if (e.compareTo(root.right.data) == 1) 
			{	root = balanceRR(root);	}
			// rotate right left
			else if (e.compareTo(root.right.data) == -1)  
			{	root = balanceRL(root);	}
		}
		// update this tree's height
		this.height = root.height;
	...
	public boolean remove​(Object o) throws ClassCastException, NullPointerException {
		...
		if ( current.left == null && current.right == null ) {
		    ...
		    // get the successor as described 
		    while ( pointer != null ) {
			...
			// if root is the node to be deleted, replace it
	...
	public void clear() {
		...
		this.root = null; this.size = 0; this.height = -1;
	}
	/**
	 ...
	 * root node. This operation is O(1).
	 ...
	 */
	public AVL<E> clone() throws CloneNotSupportedException {
	...
	/**
	 * The Node class represents the nodes of this tree. Each node holds the data of that node, 
	 * its height on the tree and a reference to its left and right nodes. Its type is generic 
	 * which means it can be used for any type of element.
	 */
	private static class Node <E> {
		...
		Node (E e) {
			data = e;
			height = -1; // height when the node is null
			left = null;
			right = null;
		}
	}
